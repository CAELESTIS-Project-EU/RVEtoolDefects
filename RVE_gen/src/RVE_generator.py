...
from RVE_gen.src.Scripts.RVE_generator.Rand_uSTRU_f_Loop import RandGen
import os
...
###############################################################################
def RVE_gen_start(caseName, gen_output, params_dicc):
    # Update parameters to local namespace
    # sys._getframe(1).f_locals.update(params_dicc)
    globals().update(params_dicc) # BSC
    ...
    Summary_file = os.path.join(gen_output,caseName+'_summary.txt')
    ...
    kfl_plot = params_dicc['kfl_plot']

    startTime = time.time() #Start program timing
    
    Fibre_pos = RandGen(Hybrid_type,Max_fibres,R,Vol_fibre_req,
         ...
         Check_Vf,NSquaresY,NSquaresX,os.path.join(gen_output, caseName),FirstFibreTypeRa,VariableFibreRadius,R_STDEV,R1_STDEV,
